# Character

| Name      | Background         | Race                    | Type        | Size      | Alignment         | Deity |
| --------- | -----------------  | ----------------------- | ----------- | --------- | ----------------- | ----- |
| Dusty     | Cloistered Scholar | Custom lineage human    | Humanoid    | Medium    | Neutral Good      |       |

| Class       | Subclass                        | Level   | HD    |
| ----------- | ------------------------------- | ------: | ----- |
| Wizard      | School of Divination            | 17      | 1d6   |

## Stats

| Stat  | Value    | Mod        | Save                         | Mods  |
| ----- | -------: | ---------: | -----:                       | ----: |
| Cha   | 10       | +0         |                              |       |
| Int   | 20       | +5         | Proficient                   |       |
| Dex   | 12       | +1         |                              |       |
| Wis   | 14       | +2         | Proficient                   |       |
| Con   | 18       | +4         | Proficient [^feat-resilient] |       |
| Str   | 10       | +0         |                              |       |

## Skills

| Skill             | Stat   | Notes                                                |
| ----------------- | ------ | :--------------------------------------------------- |
| Acrobatics        | Dex    |                                                      |
| Animal Handling   | Wis    |                                                      |
| Arcana            | Int    | Expertise [^wizard] [^skill-expert]                  |
| Athletics         | Str    |                                                      |
| Deception         | Cha    |                                                      |
| History           | Int    | Proficient [^background-cloistered]                  |
| Insight           | Wis    |                                                      |
| Intimidation      | Cha    |                                                      |
| Investigation     | Int    | Proficient [^wizard]                                 |
| Medicine          | Wis    |                                                      |
| Nature            | Int    | Proficient [^background-cloistered]                  |
| Perception        | Wis    |                                                      |
| Performance       | Cha    |                                                      |
| Persuasion        | Cha    |                                                      |
| Religion          | Int    | Proficient [^feat-skill-expert]                      |
| Slight of Hand    | Dex    |                                                      |
| Stealth           | Dex    |                                                      |
| Survival          | Wis    |                                                      |

## Other Proficiencies & Languages

### Languages
 - Common [^lineage]
 - Draconic [^lineage]
 - Primordial [^background-cloistered]
 - Sylvan [^background-cloistered]

### Armour
 - Light armour [^wizard]

### Weapons
 - Daggers, darts, slings, quarterstaffs, light crossbows [^wizard]

### Tools
 - None

| Field                | Value                            |
| -------------------- | -------------------:             |
| Proficiency bonus    | +6                               |
| AC                   | 13 Magic studded Leather +2 Animated Shield +1 Dex |
| Initiative           | +1 Dex                           |
| Walking Speed        | 30ft [^lineage]                  |
| Passive perception   | 10+2 perception                  |

| Hitpoints   | Lost HP   | Temporary Hitpoints   |
| ----------- | --------- | --------------------- |
| 138         | 0         | 22                    |

## Abilities

### Divination Savant [^school-of-divination]

1/2 gold/time to transcribe divination spells to spellbook

### Portent / Greater Portent [^school-of-divination]

Roll three d20s after long rest. Once per turn replace any attack roll, saving throw, or ability check made by you or a creature that you can see. *before the roll*
20, 18, 5

### Expert Divination [^school-of-divination]

Divination spells restore a slot one level lower (max 5th level slot recovered)

### The Third Eye [^school-of-divination]

Once per short rest (or until incapacitated) choose:

  * Darkvision. You gain darkvision out to a range of 60 feet.
  * Ethereal Sight. You can see into the Ethereal Plane within 60 feet of you.
  * Greater Comprehension. You can read any language.
  * See Invisibility. You can see invisible creatures and objects within 10 feet of you that are within line of sight.


### Arcane recovery [^wizard]

Once per day after short rest recover half wizard level (rounded up) (+1 [^arcane_grimoire]) of level 6 or lower slot.

### Cantrip formulas [^wizard]

Swap out a wizard cantrip after long rest

### Telekinetic [^feat-telekinetic]

  * +30ft Mage hand. Invisible. No verbal/somatic
  * As bonus action can shove one visable creature within 30ft by 5ft (toward/away) (str save vs 8+prof+int bonus)

### Telepathic [^feat-telepathic]

  * Speak telepathically to creatures within 60ft. Can use known languages only.
  * Detect thoughts known. Can cast free once per long rest.

## Money

|      | 1 GP is   | Coins held   |
| ---- | --------: | -----------: |
| PP   | 1/10      | 0            |
| GP   | 1         | 747          |
| EP   | 2         | 0            |
| SP   | 10        | 0            |
| CP   | 100       | 53           |

## Inventory
 - Clothing:
   - Travellers clothes [^background-archaeologist]
   - +1 Studded Leather
   - Pith helmet
 - Weapons:
   - 1 dagger finesse 20/60 1d4 [^warlock]
 - Component pouch [^warlock]
 - Stegasaurus figure wooden painted.
 - Holy water
 - +1 Arcane Grimoire - bonus to spell attack/dc and extra slot regained via arcane recovery [^arcane_grimoire] [^attuned]
 - Animated shield (+2 AC no hands in use) [^attuned]
 - Ring of Regeneration 1d6 HP every 10 minutes if you have 1 hp if you have >=1hp. Regrow parts [^attuned]
 - Souvenier tuning fork from pseudodragon spa island
 - Bratty bag of holding

## Misc

### Personality Traits
 * I'm in awe of Professor Rex

### Ideals
 * Beauty. What is beautiful points us beyond itself toward what is true.

### Bonds
 * I want to expand the sum of human knowledge

### Flaws
 * Most people scream and run when they see a demon. I stop and take notes on its anatomy.

### Backstory
TODO

## Features&Traits

### Character appearance

| Feature   | Description    |
| --------- | -------------- |
| Age       | 20             |
| Height    | 5'10"          |
| Weight    | 80kg           |
| Eyes      | Green          |
| Skin      | White          |
| Hair      | Mousy Blonde   |

## Spells

### Wizard [^warlock]

| Feature                | Value                            | Mods    |
| ---------------------- | ----------------                 | ------- |
| Spellcasting ability   | Charisma                         |         |
| Spell save DC          | 8+prof+5 cha+1[^arcane_grimoire] |         |
| Spell attack bonus     | prof+5 cha+1[^arcane_grimoire]   |         |

### Cantrips
  - Minor Illusion [^wizard]
  - Toll the Dead (60ft wis save or 4d8 necrotic, 4d12 if missing any hp) [^wizard]
  - Mage hand [^wizard] +30ft range [^feat-telekinetic]
  - Mending [^wizard]
  - Prestidigitation [^wizard]

### Spellbook
  Asterisk denotes memorised (int mod + wiz level)
  - Level 1 (Types covered: acid cold fire force lightning poison thunder):
    - Absorb elements (react) *
    - Alarm
    - Detect Magic (rit)
    - Find Familiar (rit)
    - Mage Armour
    - Magic Missile *
    - Shield (react) *
    - Sleep
  - Level 2:
    - Detect Thoughts [^feat-telepathic]
    - Mirror Image
    - Misty Step
    - Suggestion
    - Web
  - Level 3:
    - Counterspell *
    - Dispel Magic *
    - Hypnotic Pattern
    - Leomund’s Tiny Hut
  - Level 4:
    - Banishment
    - Greater Invisibility 
    - Polymorph
    - Stone shape
  - Level 5:
    - Creation
    - Geas
    - Skill Empowerment
    - Telekenesis
  - Level 6:
    - Contingency
    - Disintegrate
    - Globe of Invulnerability
    - Programmed Illusion
  - Level 7:
    - Draconic Transformation
    - Reverse Gravity
    - Simulacrum
    - Teleport
  - Level 8:
    - Antimagic Field
    - Clone
    - Dominate Monster
    - Illusory Dragon
  - Level 9:
    - True Polymorph
    - Wish

###Slots

#### Level 1
  - :o: :o: :o: :o: Regular spells

#### Level 2
  - :o: Detect Thoughts [^feat-telepathic]
  - :o: :o: :o: Regular spells

#### Level 3
  - :o: :o: :o: Regular spells

#### Level 4
  - :o: :o: :o: Regular spells

#### Level 5
  - :o: :o: Regular spells

#### Level 6
  - :o: Regular spells

#### Level 7
  - :o: Regular spells

#### Level 8
  - :o: Regular spells

#### Level 9
  - :o: Regular spells

## Notes

[^wizard]: Wizard class
[^school-of-divination]: Divination wizard class
[^lineage]: Custom lineage human
[^background-archaeologist]: Archaeologist background from Tomb of Annihilation
[^attuned]: Attuned items (3)
[^arcane_grimoire]: Item

